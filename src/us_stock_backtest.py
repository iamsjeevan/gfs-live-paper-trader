...
SLIPPAGE = 0.001          # 0.1% transaction cost for US liquid equities

# Top US Large/Mid Cap Stocks Universe (S&P 500 Leaders across Tech, Healthcare, Industrial, Consumer)
US_TICKERS = [
    'AAPL', 'MSFT', 'NVDA', 'GOOGL', 'AMZN', 'META', 'TSLA', 'AMD', 'AVGO', 'NFLX',
    'COST', 'LLY', 'PLTR', 'UNH', 'JNJ', 'V', 'MA', 'HD', 'PG', 'ABBV',
    'MRK', 'KO', 'PEP', 'BAC', 'WMT', 'ORCL', 'CSCO', 'ACN', 'MCD', 'DIS',
    'LIN', 'ADBE', 'PM', 'TMO', 'CRM', 'ABT', 'NKE', 'TXN', 'DHR', 'QCOM',
    'INTC', 'HON', 'AMAT', 'UNP', 'LOW', 'SPGI', 'CAT', 'GS', 'IBM', 'GE',
    'NOW', 'ISRG', 'BKNG', 'AMGN', 'SYK', 'MDLZ', 'TJX', 'REGN', 'LRCX', 'PFE'
]

def download_us_prices():
    print(f"[US DOWNLOAD] Downloading historical daily prices for {len(US_TICKERS)} US stocks + S&P 500 benchmark (^GSPC)...", flush=True)
    all_tickers = US_TICKERS + ['^GSPC']
    ...
    print(f"[US DOWNLOAD OK] Price matrix downloaded: {close_df.shape[0]} trading days across {close_df.shape[1]} tickers.", flush=True)
    return close_df

def run_us_backtest(close_df, num_stocks=10):
    # Benchmark
    sp50_prices = close_df['^GSPC'].dropna()
    stock_prices = close_df[US_TICKERS].dropna(how='all').ffill().bfill()
    ...
    for i in range(len(monthly_dates) - 1):
        ...
        for sym, pos in current_positions.items():
            p = stock_prices.loc[rebal_date, sym] if (rebal_date in stock_prices.index and sym in stock_prices.columns and pd.notna(stock_prices.loc[rebal_date, sym])) else pos['entry_price']
        ...
        for s in available_symbols:
            if s in mom_3m_pivot.columns:
                ...
                p = stock_prices.loc[rebal_date, s]
        ...
        for s in dropped:
            ...
            p = stock_prices.loc[rebal_date, s] if (rebal_date in stock_prices.index and s in stock_prices.columns and pd.notna(stock_prices.loc[rebal_date, s])) else pos['entry_price']
    ...
    for s, pos in current_positions.items():
        p = stock_prices.loc[final_date, s] if (final_date in stock_prices.index and s in stock_prices.columns and pd.notna(stock_prices.loc[final_date, s])) else pos['entry_price']
    ...
    bench_units = INITIAL_CAPITAL / sp50_prices.loc[monthly_dates[0]]
    ...
    bench_final = bench_units * sp50_prices.loc[monthly_dates[-1]]
...
def main():
    print("=" * 85, flush=True)
    print("      US STOCK MARKET (S&P 500) QUALITY-MOMENTUM STRATEGY BACKTEST      ", flush=True)
    print("=" * 85, flush=True)

    close_df = download_us_prices()
    res10 = run_us_backtest(close_df, num_stocks=10)
    res20 = run_us_backtest(close_df, num_stocks=20)

    print("\n--- 1. US STRATEGY PERFORMANCE VS S&P 500 BENCHMARK (2019 - 2026) ---", flush=True)
    print(f"Total Invested Capital:              ${res10['total_invested']:,.0f} ($10k Initial + $10k Annual SIP)")
    
    print(f"\n10-Stock US Quality Momentum:")
