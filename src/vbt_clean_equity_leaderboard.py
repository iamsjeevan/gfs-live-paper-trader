...
def load_clean_data():
    ...
    pivoted_high = df.pivot(index='date', columns='symbol', values='high').ffill().bfill().astype(float)
    pivoted_low = df.pivot(index='date', columns='symbol', values='low').ffill().bfill().astype(float)

    nifty_merged = pd.DataFrame({'date': pivoted_close.index}).merge(nifty_df, on='date', how='left').ffill()
    nifty_close = nifty_merged['nifty_close'].astype(float)

    return pivoted_close, pivoted_high, pivoted_low, nifty_close, stocks

def backtest_strategy(pivoted_close, pivoted_high, pivoted_low, nifty_close, stocks, mode):
    ...
    # Nifty 200 SMA Regime Shield (Go 100% Cash when Nifty < 200 SMA)
    nifty_bull = (nifty_close > nifty_sma200).bfill().values
...
def main():
    ...
    p_close, p_high, p_low, nifty_close, stocks = load_clean_data()
    ...
    for mode, name in strats:
        vbt_stats, final_val, cagr, max_dd = backtest_strategy(p_close, p_high, p_low, nifty_close, stocks, mode)
        print(f"\n==========================================================================================")
