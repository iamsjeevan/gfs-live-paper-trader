...
def run_long_short_backtest(df, symbol="BTC", breakout_window=15, max_hold_days=5, allow_short=True):
    df_sim = df.copy()
    ...
    for i in range(200, len(df_sim)):
        ...
        if in_trade:
            ...
            # Long Entry: Price > 50 SMA AND Price > Breakout High
            ...
            # Short Entry: Price < 50 SMA AND Price < Breakout Low
...
def main():
    print("=" * 95, flush=True)
    print("      LONG vs LONG + SHORT CRYPTO SWING TRADING BACKTEST AUDIT (BTC & ETH)      ", flush=True)
    print("=" * 95, flush=True)

    btc_df = load_crypto_data("BTC")
    eth_df = load_crypto_data("ETH")

    print("\n--- 1. BITCOIN (BTC-USD) COMPARISON (2014 - 2026) ---")
    btc_long = run_long_short_backtest(btc_df, symbol="BTC", breakout_window=15, max_hold_days=5, allow_short=False)
    btc_both = run_long_short_backtest(btc_df, symbol="BTC", breakout_window=15, max_hold_days=5, allow_short=True)
    ...
    print("\n--- 2. ETHEREUM (ETH-USD) COMPARISON (2017 - 2026) ---")
    eth_long = run_long_short_backtest(eth_df, symbol="ETH", breakout_window=30, max_hold_days=5, allow_short=False)
    eth_both = run_long_short_backtest(eth_df, symbol="ETH", breakout_window=30, max_hold_days=5, allow_short=True)
