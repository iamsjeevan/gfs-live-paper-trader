...
def run_strategy(df, nifty_df, stocks, strat_type):
    ...
    nifty_bull_series = (nifty_close > nifty_sma200).fillna(False)
    ...
    for i in range(200, len(dates)):
        curr_d = dates[i]
        is_nifty_bull = bool(nifty_bull_series.iloc[i])
        ...
        for sym, pos in list(active_positions.items()):
            ...
            # Market regime shield exit OR technical exit
        ...
        # 2. Process Entries (Max 10 Positions, 10% Capital Allocation Each)
            ...
            for s in stocks:
                if s not in active_positions:
                    ...
                    if pd.notna(cp) and pd.notna(s50v) and pd.notna(s200v):
                        if strat_type == "VCP":
                            # Minervini VCP: Volatility contraction + Trend
                            ...
                            if cp > s50v and cp > s200v and pd.notna(r6) and r6 > 0.15:
                                candidates.append((s, r6))
                        elif strat_type == "EMA_PULLBACK":
                            # 20 EMA Pullback Bounce in Uptrend
                            if cp > s200v and cp > s20v and cp > s50v:
                                candidates.append(s)

            if candidates:
                if strat_type == "DUAL_MOMENTUM":
                    # Sort candidates by highest 6-month momentum
                    candidates.sort(key=lambda x: x[1], reverse=True)
                    selected = [x[0] for x in candidates[:slots]]
                else:
                    selected = candidates[:slots]

                tot_val = cash + sum([pos['shares'] * c_row[sym] for sym, pos in active_positions.items() if pd.notna(c_row[sym])])
                target_alloc = tot_val * 0.10 # 10% per stock slot
...
def main():
    ...
    for code, name in strats:
        vbt_stats, final_val, cagr, max_dd = run_strategy(df, nifty_df, stocks, code)
        print(f"\n==========================================================================================")
