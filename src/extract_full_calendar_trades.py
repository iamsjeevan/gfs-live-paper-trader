...
def main():
    print("Fast Extracting Day-by-Day Calendar Dataset...")
    df, fno_symbols = load_fno_data()

    # Pre-build candle dicts for superfast lookup
    candle_dict = {}
    ...
    for i in range(50, len(dates)):
        ...
        if active_symbols:
            ...
            for s in active_symbols:
                ...
                # Extract candle window from pre-built dict
    ...
    print(f"✅ Extracted {len(calendar_days)} Trading Days")
    print(f"🟢 Best Month:  {best_month[0]} (+₹{best_month[1]:,.2f} profit)")
    print(f"🔴 Worst Month: {worst_month[0]} (₹{worst_month[1]:,.2f} loss)")
    ...
    with open("strategy_showcase/public/calendar_dataset.json", "w") as f:
        json.dump(payload, f, indent=2)

    print("Saved strategy_showcase/public/calendar_dataset.json successfully!")
