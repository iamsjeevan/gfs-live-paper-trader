...
def run_risk_sizing_sim(target_portfolio_risk_pct=1.0, stock_stop_loss_pct=1.0):
    ...
    for i in range(50, len(dates)):
        ...
        if top_picks:
            # Risk Sizing Math:
            # Target Portfolio Risk = target_portfolio_risk_pct % of Cash
            # Total Capital Risk Allowed Today (₹) = cash * (target_portfolio_risk_pct / 100.0)
            # Risk per trade (₹) = Total Capital Risk Allowed Today / len(top_picks)
            # Position Size per trade (₹) = Risk per trade / (stock_stop_loss_pct / 100.0)
            
            total_capital_risk_rupees = cash * (target_portfolio_risk_pct / 100.0)
            risk_per_trade_rupees = total_capital_risk_rupees / len(top_picks)
            position_size_per_trade_rupees = risk_per_trade_rupees / (stock_stop_loss_pct / 100.0)

            # Implied Total Account Leverage
            total_account_exposure = position_size_per_trade_rupees * len(top_picks)
            implied_leverage = total_account_exposure / cash

            daily_pnl = 0.0
            for s in top_picks:
                drop_pct = (l_row[s] - o_row[s]) / o_row[s]
                
                # Check 1% Stock Stop Loss
                if drop_pct <= -(stock_stop_loss_pct / 100.0):
                    ret = -(stock_stop_loss_pct / 100.0) - STT_BROKERAGE_SLIPPAGE
                else:
                    ret = ((c_row[s] - o_row[s]) / o_row[s]) - STT_BROKERAGE_SLIPPAGE

                daily_pnl += position_size_per_trade_rupees * ret
    ...
    return {
        "portfolioRiskPct": f"{target_portfolio_risk_pct}% Portfolio Risk",
        "impliedLeverage": f"{(target_portfolio_risk_pct / stock_stop_loss_pct):.1f}x Leverage",
...
def main():
    print("=" * 100)
    print("      FIXED FRACTIONAL ACCOUNT RISK SIZING AUDIT (INTRADAY 1% STOP LOSS)      ")
    print("=" * 100)

    r1 = run_risk_sizing_sim(target_portfolio_risk_pct=1.0, stock_stop_loss_pct=1.0)
    r2 = run_risk_sizing_sim(target_portfolio_risk_pct=2.0, stock_stop_loss_pct=1.0)
    r3 = run_risk_sizing_sim(target_portfolio_risk_pct=3.0, stock_stop_loss_pct=1.0)
    r5 = run_risk_sizing_sim(target_portfolio_risk_pct=5.0, stock_stop_loss_pct=1.0)

    for r in [r1, r2, r3, r5]:
        print(f"\n📌 Account Risk Target: {r['portfolioRiskPct']} (Implied Account Exposure: {r['impliedLeverage']})")
        print(f"   - Final Portfolio Value (₹1 Lakh Start): ₹{r['finalVal']:,.2f}")
